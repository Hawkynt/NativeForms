...
- **Platform limits, documented honestly.** Multiline placeholders are owner-drawn: neither Win32's cue banner (`EM_SETCUEBANNER`, single-line `EDIT` only) nor `GtkTextView` offers a native one, so each backend paints it after the widget's own draw while the box is empty — GTK off the view's `draw` signal, Win32 out of the `EDIT` subclass on `WM_PAINT`, into the control's own formatting rectangle so the hint sits where the first typed character will. It stays visible while the box has focus on both, which a cue banner would not. `MaxLength` and password masking are also entry-only on GTK (`GtkTextView` has no native limit or masking).
...
- Not yet implemented (see [docs/PRD.md](../PRD.md) §7.3): word-wrap control and an undo API.
