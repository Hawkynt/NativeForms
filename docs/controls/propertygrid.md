...
**Driving a [`DataGridView`](datagridview.md) or a [`ListView`](listview.md) from the same attributes** ships: one `[GridEditable]` model emits `PopulateGrid(PropertyGrid)`, `PopulateColumns(DataGridView)`, `PopulateColumns(ListView)` and `ToListViewItem()`, so the same annotations give you an inspector, a grid and a list. The remaining parity items — click behaviour, per-cell styling and conditional images — are specified in [PRD §15](../PRD.md#15-attribute-driven-grids--lists--extend-the-generator-to-datagridview).
