...
- **The committed value is a file, never a directory.** In `Open` mode a typed (or otherwise supplied) path that resolves to an existing *directory* is refused outright: it never becomes `SelectedPath`, the previously committed value stands, and the editor snaps back to it — exactly as WinForms' `OpenFileDialog` navigates *into* a typed folder instead of returning it. A *missing file* path is not refused; it commits and is merely flagged broken by the warning frame, since it may be a typo worth seeing or a file about to appear. `Save` refuses nothing here — naming a not-yet-written file is the point. When a directory is the wanted value, use [`FolderPicker`](folderpicker.md).
...
- `FilePickerTests` pin the surface headlessly: editor placement, the dialog options handed over, multi-select bookkeeping, Enter commit, both existence semantics, the Open-mode directory veto (a typed folder refused, the previous file kept, a real file accepted), the Save-mode acceptance of a fresh name, the warning frame and the disabled state.
