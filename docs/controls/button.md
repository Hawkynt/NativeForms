...
- **The promotion gate.** An image *on its own* keeps the widget: every platform centres a bare bitmap on a button (GTK `gtk_button_set_image`, Win32 `BM_SETIMAGE`/`BS_BITMAP`, AppKit's image cell), so all three agree. An image **with a caption beside it** does not — GTK places it, a classic Win32 `BUTTON` renders the bitmap alone and drops the caption outright, AppKit has its own idea. That face, and `FlatStyle.Flat`/`Popup`, give up the widget and are painted through the same `ContentLayout` geometry [`Label`](label.md), [`CheckBox`](checkbox.md) and [`GroupBox`](groupbox.md) use, so they come out identical everywhere. The swap runs both ways on a live control and is invisible to the application; `IsNativeWidget` is the only way to tell.
