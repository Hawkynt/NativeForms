...
WinForms is a good API trapped on one operating system. NativeForms keeps the API — `Form`, `Button`,
a `Controls` collection, `Click` — and replaces what is underneath: real native widgets where the
platform has one worth using, owner-drawn controls in the host's own visual style where it does not.

The target is that a WinForms program should port by changing its using directives, and then look
like it belongs on whatever it is running on. `docs/PRD.md` tracks the distance still to go,
box by box, rather than leaving the gap to be discovered.
