...
public class TreeView : OwnerDrawnControl
{
    ...
    private readonly List<TreeNode> _flat = [];
    ...
    public TreeView() => this.Nodes = new(this);
    ...
    public TreeNode? SelectedNode
    {
        ...
        set
        {
            ...
            if (value is not null && !ReferenceEquals(value.TreeView, this))
    ...
    /// <summary>Expands a node through the cancelable Before/After event pipeline.</summary>
    internal void RequestExpand(TreeNode node)
    {
        if (node.IsExpanded)
            return;

        var e = new TreeViewCancelEventArgs(node);
        ...
        this.OnAfterExpand(new TreeViewEventArgs(node));
    }

    /// <summary>Collapses a node through the cancelable Before/After event pipeline.</summary>
    internal void RequestCollapse(TreeNode node)
    {
        if (!node.IsExpanded)
            return;
        ...
        this.OnStructureChanged();
        if (this.IsSelectionUnder(node))
            this.SelectedNode = node;

        this.OnAfterCollapse(new TreeViewEventArgs(node));
    }

    /// <summary>Whether the selected node is a descendant of (and now hidden under) the given node.</summary>
    private bool IsSelectionUnder(TreeNode node)
    {
        for (var ancestor = _selectedNode?.Parent; ancestor is not null; ancestor = ancestor.Parent)
    ...
    {
        _flatDirty = true;
        if (_selectedNode is not null && !ReferenceEquals(_selectedNode.TreeView, this))
            _selectedNode = null;
    ...
    private void EnsureFlat()
    {
        ...
        _flat.Clear();
        this.AppendVisible(this.Nodes);
        _flatDirty = false;
        this.ClampScroll();
    }

    private void AppendVisible(TreeNodeCollection nodes)
    {
        for (var i = 0; i < nodes.Count; ++i)
        {
            ...
            if (node.IsExpanded && node.HasChildren)
                this.AppendVisible(node.Nodes);
        }
    }

    private void ClampScroll()
    {
        var maxTop = Math.Max(0, _flat.Count - this.VisibleRowCount);
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        var index = _selectedNode is null ? -1 : _flat.IndexOf(_selectedNode);
        var node = _selectedNode;

        var handled = true;
        switch (e.KeyCode)
        {
            case Keys.Down when count > 0: this.SelectRow(Math.Min(count - 1, index + 1)); break;
            case Keys.Up when count > 0: this.SelectRow(Math.Max(0, index - 1)); break;
            case Keys.Home when count > 0: this.SelectRow(0); break;
            case Keys.End when count > 0: this.SelectRow(count - 1); break;
            case Keys.PageDown when count > 0: this.SelectRow(Math.Min(count - 1, index + this.VisibleRowCount)); break;
            case Keys.PageUp when count > 0: this.SelectRow(Math.Max(0, index - this.VisibleRowCount)); break;
            case Keys.Right when node is not null:
                if (!node.HasChildren)
                    break;

                if (node.IsExpanded)
                    this.SelectedNode = node.Nodes[0];
                else
                    this.RequestExpand(node);

                break;
            case Keys.Left when node is not null:
                if (node.IsExpanded && node.HasChildren)
                    this.RequestCollapse(node);
                else if (node.Parent is not null)
                    this.SelectedNode = node.Parent;

                break;
            case Keys.Add when node is not null: this.RequestExpand(node); break;
            case Keys.Subtract when node is not null: this.RequestCollapse(node); break;
            case Keys.Multiply when node is not null: node.ExpandAll(); break;
            case Keys.Space when node is not null:
                ...
                break;
            case Keys.Enter when node is not null: node.Toggle(); break;
    ...
    private void SelectRow(int index) => this.SelectedNode = _flat[Math.Clamp(index, 0, _flat.Count - 1)];
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        var last = Math.Min(_flat.Count, _topIndex + this.VisibleRowCount + 1);
        for (var i = _topIndex; i < last; ++i)
            this.PaintRow(g, theme, _flat[i], (i - _topIndex) * rowHeight, rowHeight);
    ...
    private void PaintRow(IGraphics g, ITheme theme, TreeNode node, int y, int rowHeight)
    {
        ...
        if (this.ShowPlusMinus && node.HasChildren && glyphCellLeft >= 0)
            PaintGlyph(g, theme, node, glyphCellLeft, y, indent, rowHeight);
    ...
    private static void PaintGlyph(IGraphics g, ITheme theme, TreeNode node, int cellLeft, int y, int indent, int rowHeight)
    {
        // The classic themed square with a − (expanded) or + (collapsed) inside.
        var box = new Rectangle(cellLeft + ((indent - _GlyphBox) / 2), y + ((rowHeight - _GlyphBox) / 2), _GlyphBox, _GlyphBox);
        g.FillRectangle(theme.FieldBackground, box);
        g.DrawRectangle(theme.Border, box);

        var midX = box.X + (_GlyphBox / 2);
        var midY = box.Y + (_GlyphBox / 2);
        g.DrawLine(theme.ControlText, box.X + 2, midY, box.X + _GlyphBox - 2, midY);
        if (!node.IsExpanded)
            g.DrawLine(theme.ControlText, midX, box.Y + 2, midX, box.Y + _GlyphBox - 2);
    }
