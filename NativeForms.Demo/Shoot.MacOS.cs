...
internal static unsafe partial class ShootMacOS
{
    ...
    /// <summary>Captures the application's main window, returning the size written or null.</summary>
    public static Size? Window(string path)
    {
        ...
        var window = Send(app, sel_registerName("mainWindow"));
        if (window == 0)
            window = Send(app, sel_registerName("keyWindow"));
